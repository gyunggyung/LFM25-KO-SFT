...
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)_stage0_legal_lfmchat}"
...
DATASET_PATH="${DATASET_PATH:-$DATA_ROOT/prepared/lfm_chat/20260628_lfmchat_stage0_legal}"
...
"$VENV/bin/accelerate" launch --num_processes 8 --mixed_precision bf16 \
  ...
  --stage-name stage0_legal_lfmchat \
  --max-seq-length 8192 \
