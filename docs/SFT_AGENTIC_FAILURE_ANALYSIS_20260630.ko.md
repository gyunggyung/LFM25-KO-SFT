...
CPU-only repair 준비 코드는 `docs/CPT_REPAIR_SFT_PLAN_20260630.ko.md`에 정리했다.
dry-run 결과는 raw 700 rows, tokenized 329,767 tokens, answer-only ratio 28.71%,
token max 124,900 / vocab 125,017, empty response 0으로 통과했다. 학습 launcher는
`ALLOW_TRAIN=1` 없이는 실행되지 않는다.
